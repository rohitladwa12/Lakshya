...
function buildOrchestratedStep($type, $questionNum, $totalQs, $message, $aiService)
{
    ...
    if ($phase === 'APTITUDE') {
        ...
        // If the AI message contains real A)–D) options, activate the dedicated
        // MCQ panel (previously dead code — the orchestrator never emitted it).
        $parsed = parseMCQOptions($message);
        if (count($parsed['options']) >= 2 && trim($parsed['body']) !== '') {
            $step["ui"] = "mcq";
            $step["components"] = ["chat_window", "mcq_viewer", "timer"];
            $step["question"] = [
                "body" => $parsed['body'],
...
function parseMCQOptions($text)
{
    ...
    // No fabricated placeholder options when parsing fails — an empty array
    // tells the caller this message is not a renderable MCQ.
...
try {
    switch ($action) {
        ...
        case 'start':
            ...
            $sql = "INSERT INTO mock_ai_interview_sessions (student_id, role_name, status, institution, concept, company_name, difficulty) VALUES (?, ?, 'active', ?, ?, ?, ?)";
            $stmt = $db->prepare($sql);
            $stmt->execute([$studentIdForDb, $role, $institution, $concept, $company, $difficulty]);
            ...
            $welcomeMsg = "Hi, welcome to your Mock AI Interview focusing on **$role** (Difficulty: **$difficulty**). We will start with the **$type** round. Please let me know when you are ready to begin.";
        ...
        case 'chat':
            ...
            // Aptitude questions are dynamically generated by AI based on candidate's concept
            $aptitudeQuestions = [];
            ...
            session_write_close();
            $response = $aiService->getTechnicalInterviewResponse($role, $trimmedHistory, $profile, '', $type, $projects, $aptitudeQuestions, $concept, $company, $difficulty);
        ...
        case 'save_pdf':
            ...
            if (move_uploaded_file($_FILES['report_pdf']['tmp_name'], $destination)) {
                ...
                error_log("mock_ai save_pdf: move_uploaded_file failed for $destination");
                ob_clean();
    ...
    // Any uncaught exception previously rendered an HTML fatal, which the
    // frontends could not parse — always answer with JSON.
    error_log("Mock AI Handler Exception [$action]: " . $e->getMessage() . "\n" . $e->getTraceAsString());
    ob_clean();
